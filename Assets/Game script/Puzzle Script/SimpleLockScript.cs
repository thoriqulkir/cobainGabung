...
public class SimpleLockScript : MonoBehaviour
{
    ...
    public string puzzleIdentifier;
    public string sceneToLoadAfterCompletion;

    void Start()
    {
        // Muat status puzzle dari PlayerPrefs menggunakan identifier unik
        isPuzzleSolved = PlayerPrefs.GetInt(puzzleIdentifier + "_PuzzleSolved", 0) == 1;

        hasInteractedBefore = PlayerPrefs.GetInt(puzzleIdentifier + "_InteractedBefore", 0) == 1;
    ...
    public void Unlock()
    {
        interactable = false;
        isPuzzleSolved = true;
        PlayerPrefs.SetInt(puzzleIdentifier + "_PuzzleSolved", 1); // Simpan status puzzle
        PlayerPrefs.SetString(puzzleIdentifier + "_SceneToLoad", sceneToLoadAfterCompletion);
        PlayerPrefs.Save();
        StopInteract();
        ...
        if (interactable)
        {
            if (!hasInteractedBefore)
            {
                ...
                if (monologPuzzle != null)
                {
                    ...
                    PlayerPrefs.SetInt(puzzleIdentifier + "_InteractedBefore", 1); // Simpan status interaksi pertama kali
                    PlayerPrefs.Save();
                }
                else
                {
